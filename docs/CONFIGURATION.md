...
| `VITE_API_BASE` | when `VITE_USE_MOCKS=false` | Base URL of the running API, e.g. `http://localhost:3000`. In production, Caddy proxies `/api` and this can point at the same origin. |
