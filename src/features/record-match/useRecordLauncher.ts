// Opening the record form from anywhere: the desktop rail's Record button, `R`, LATEST's "Record
// match", "Record with {name}" on a profile, and /record visited on a desktop window. Where the form
// lives depends on the screen (DESIGN-SPEC.md §6):
//   - phone/tablet: the full-screen /record route (`?home=` pre-fills Home)
//   - desktop: the Record drawer, on every page (4b; it replaced 3a's docked panel)
//
...
/** Open the desktop drawer, with a Home pre-fill and a focus request for its form. Plain function
 *  so the router guard can call it outside any component. */
